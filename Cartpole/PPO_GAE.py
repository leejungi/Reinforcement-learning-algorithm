...
class DDQN:
	...
	#Update
	def train(self, states, next_states, actions, rewards, probs):
		...
		#Compute returns
		with torch.no_grad():
			returns = rewards + self.discount_factor * self.Critic(next_states)
		...
		if GAE == True:
			A = 0
			Advantage = []
			returns = (returns - self.Critic(states)).detach()
...
if __name__ =="__main__":
	...
	states, next_states, actions, rewards, probs = [], [], [], [], []
	train_interval_ep =4
	train_count =0
	#Training		 
	for i in range(num_episode):
		...
		if train_count % train_interval_ep ==0:
			train_count =0
			policy.train(states, next_states, actions, rewards, probs)
